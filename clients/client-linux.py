...
def get_load():
	...
	if system[0][:6] == "CentOS":
		if system[1][0] == "6":
			tmp_load = os.popen("netstat -anp |grep ESTABLISHED |grep '::ffff:' |awk '{print $5}' |awk -F ':' '{print $4}' |sort -u |grep -E -o '([0-9]{1,3}[\.]){3}[0-9]{1,3}' |wc -l").read()
		else:
			tmp_load = os.popen("netstat -anp |grep ESTABLISHED |awk '{print $5}' |awk -F ':' '{print $1}' |sort -u |grep -E -o '([0-9]{1,3}[\.]){3}[0-9]{1,3}' |wc -l").read()
	else:
		tmp_load = os.popen("netstat -anp |grep ESTABLISHED |awk '{print $5}' |awk -F ':' '{print $1}' |sort -u |grep -E -o '([0-9]{1,3}[\.]){3}[0-9]{1,3}' |wc -l").read()
